...
---------------------
This is a local RPA-style pipeline: Excel provides the work queue, Playwright
controls a user-authenticated Sumsub browser, Google Drive API stores results,
and a JSON manifest makes the workflow resumable. It does *not* use computer
vision or AI: the browser locates the ``Get PDF`` control through page DOM
semantics (accessible role and text).
...
REQUIRED_HEADERS = {
    ...
    "drive_url": ("google drive pdf url", "google_drive_url", "google drive url"),
}
...
def get_drive_service(credentials_path: Path, token_path: Path):
    ...
    if not credentials or not credentials.valid:
        if not credentials_path.exists():
            raise RuntimeError(
                "Google OAuth credentials file was not found. See docs/GOOGLE_OAUTH.md."
            )
...
def find_headers(sheet: Any) -> dict[str, int]:
    found: dict[str, int] = {}
    aliases = {
        key: {normalise_header(label) for label in labels}
        for key, labels in REQUIRED_HEADERS.items()
    }
    for column in range(1, sheet.max_column + 1):
        ...
        for key, targets in aliases.items():
            if header in targets:
                found[key] = column
    missing = [REQUIRED_HEADERS[key][0] for key in REQUIRED_HEADERS if key not in found]
    if missing:
...
def cell_value_for_reading(sheet: Any, values_sheet: Any, row: int, column: int) -> Any:
    """Prefer Excel's cached calculation result while retaining plain literal cells."""
    value = values_sheet.cell(row, column).value
    if value is not None:
        return value
    raw_value = sheet.cell(row, column).value
    if isinstance(raw_value, str) and raw_value.startswith("="):
        return None
    return raw_value


def build_row_index(sheet: Any, values_sheet: Any, columns: dict[str, int]) -> OrderedDict[str, list[int]]:
    rows: OrderedDict[str, list[int]] = OrderedDict()
    for row in range(2, sheet.max_row + 1):
        applicant_id = str(cell_value_for_reading(sheet, values_sheet, row, columns["id"]) or "").strip()
        source_url = str(cell_value_for_reading(sheet, values_sheet, row, columns["sumsub_url"]) or "").strip()
...
def is_marked_not_found(sheet: Any, rows: list[int], column: int) -> bool:
    """A deliberate 'not found' in the Drive URL column is a terminal, user-reviewed status."""
    values = [str(sheet.cell(row, column).value or "").strip().casefold() for row in rows]
    return bool(values) and all(value == "not found" for value in values)
...
def set_row_values(sheet: Any, rows: list[int], drive_column: int, status_column: int, url: str, status: str) -> None:
    for row in rows:
        ...
        cell.value = url
        cell.hyperlink = None if url.strip().casefold() == "not found" else url
...
def download_sumsub_pdf(page: Page, source_url: str, destination: Path) -> None:
    ...
    # Sumsub may client-side redirect to its 404 page shortly after DOMContentLoaded.
    page.wait_for_timeout(1_500)
    if "/404/" in page.url:
        raise RuntimeError("Sumsub reports: Applicant not found.")
    try:
        unavailable = page.get_by_text(re.compile(r"applicant\s+not\s+found", re.IGNORECASE))
        if unavailable.first.is_visible(timeout=1_500):
            raise RuntimeError("Sumsub reports: Applicant not found.")
...
def choose_source_url(sheet: Any, values_sheet: Any, rows: list[int], column: int) -> str:
    value = cell_value_for_reading(sheet, values_sheet, rows[0], column)
    source_url = str(value or "").strip()
    if not source_url:
        raise RuntimeError(
            "Sumsub_Url is a formula without a saved calculation result. Open the input workbook in "
            "Microsoft Excel, press Cmd+S to save it, close Excel, then rerun the command."
        )
    if source_url.startswith("="):
        raise RuntimeError(
            "Sumsub_Url is an Excel formula rather than a usable URL. Open the input workbook in "
            "Microsoft Excel, press Cmd+S to save it, close Excel, then rerun the command."
        )
    return source_url
...
def main() -> int:
    ...
    indexed_rows = build_row_index(sheet, values_sheet, columns)
    print(f"Found {len(indexed_rows)} unique Sumsub IDs with a Sumsub_Url.")
    ...
    try:
        ...
        for applicant_id, rows in selected:
            ...
            try:
                ...
                if "Applicant not found" in message:
                    record.update({"status": "not_found", "error": message, "updated_at": utc_now()})
                    set_row_values(sheet, rows, columns["drive_url"], status_column, "not found", "Not found: Sumsub report unavailable")
                else:
